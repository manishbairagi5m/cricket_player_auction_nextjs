...
export default function TournamentDetail() {
  ...
  const handleTabs = (item: any, i: number) => {
    ...
    if (findTab) {
      router.push(
        `/user/tournament/detail/${router.query.id}?tab=${findTab.key}`
      );
    }
  };
        ...
        <ul>
          {obj.map((item: any, i: number) => (
            <li
              style={{
                color: `${
                  currentPlate >= 0 && currentPlate === i ? "#191966" : "black"
                }`,
              }}
              onClick={() => handleTabs(item, i)}
              key={i}
            >
              {item.label}
            </li>
          ...
          {currentPlate >= 0 && (
            <div
              className="tournament-current-plate"
