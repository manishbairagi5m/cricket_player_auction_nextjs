import {
  ...
  import { AddTeamAdmin } from "@/customApi/tournament";
...
import teamJson from "@/Components/Tournament/Teams/team.json"  
  ...
  //   players: [],
  // };
const initialValues = teamJson[4];

  const AddTeam = ({ state, setAdd, getDetail }) => {
    ...
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        ...
        data.append("tournament_id", router.query.id);
        data.append("country_code", '91');
        // data.append("add_me", addMe);
        if (newData.length < state.min_players) {
            setLoader(false)
            toast.error(`Please Select Minimum ${state.min_players}`);
        } else if (newData.length > state.max_players) {
            setLoader(false)
            toast.error(`Only ${state.max_players} Players Can Be Selected`);
        } else {
            console.log(newData,'newData')
          await AddTeamAdmin(data)
            .then((res) => {
              setLoader(false)
    ...
    const handleAddMe = (e) => {
      let arr = [...values.players];
      if(e.target.checked){
        if((state?.max_players-values?.players.length) === 0){
          toast.error("Player Limit Exceeded")
        }else{
          arr.unshift({ name: session.user.data.name, phone_number: session.user.data.phone_number });
          setFieldValue("players", arr);
        }
      }else{
        let newdata = arr.filter((item)=> item.name !== session.user.data.name)
        setFieldValue("players", newdata);
      }
    }
  
    const checkAddMeInclude = () => {
      let addme = values.players.find((item)=> (item.name === session.user.data.name) && (item.phone_number === session.user.data.phone_number))
      return addme ? true : false
    }
